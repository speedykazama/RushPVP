...
RegisterNetEvent('Night_Towing:openTowingMenu')
AddEventHandler('Night_Towing:openTowingMenu', function()
    OpenTowingMenu()
end)
...
function DetachRope()
    if entity1 and entity2 then
        CloseTowingMenu()
        TriggerServerEvent('Night_Towing:stopTow')
...
function AttemptAttachRope()
    if entity1 and entity2 then
        ...
        TriggerServerEvent('Night_Towing:tow', veh1, veh2)
        CloseTowingMenu()
...
RegisterNetEvent('Night_Towing:removeRope')
AddEventHandler('Night_Towing:removeRope', function(id, veh1_, veh2_)
    for k, rope in pairs(ropes) do
        if rope.id == id then
...
RegisterNetEvent('Night_Towing:makeRope')
AddEventHandler('Night_Towing:makeRope', function(veh1_, veh2_, id, owner)
    for k, rope in pairs(ropes) do
        if rope.id == id then
            DeleteRope(rope.rope)
            ropes[k] = nil
