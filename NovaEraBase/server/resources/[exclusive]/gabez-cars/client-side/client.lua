...
local vSERVER = Tunnel.getInterface("gabez-cars")
        ...
        if vehicle ~= 0 then
            if vSERVER.checkXenon() then
                ...
                if vehicleSpeed <= 25 then
                    ...
                    TriggerEvent("Notify", "verde", "Você está muito rápido!", 5000)
                end
            else
                TriggerEvent("Notify", "verde", "O veículo não possui o módulo de xenon.", 5000)
            end
        else
            TriggerEvent("Notify", "verde", "Você não está em um veículo!", 5000)
        end
    end)
    RegisterCommand(cfg.comandoNeon, function(source, args, rawCommand)
        ...
        if vehicle ~= 0 then
            if vSERVER.checkNeon() then
                ...
                TriggerEvent("Notify", "verde", "O veículo não possui o módulo de neon.", 5000)
            end
        else
            TriggerEvent("Notify", "verde", "Você não está em um veículo!", 5000)
        end
    ...
    RegisterCommand(cfg.comandoSuspensao, function(source, args, rawCommand)
        ...
        if vehicle ~= 0 then
            if vSERVER.checkSuspension(vehicle) then
                ...
                TriggerEvent("Notify", "verde", "O veículo não possui suspensão a ar!", 5000)
            end
        else
            TriggerEvent("Notify", "verde", "Você não está em um veículo!", 5000)
        end
...
RegisterNUICallback("ButtonClick", function(data, cb)
    if data == "comprar-suspensao" then
		TriggerServerEvent("departamento-comprar", "suspensionair")
	elseif data == "comprar-neon" then
		TriggerServerEvent("departamento-comprar", "moduleneon")
	elseif data == "comprar-xenon" then
		TriggerServerEvent("departamento-comprar", "modulexenon")
    ...
    if data.action == "useControl" then
        ...
        if alturaAtual < tonumber(-0.09) then
            TriggerEvent("Notify", "verde", "Altura miníma atingida!", 5000)
            return
        end
...
RegisterCommand("instalar", function(source, args, rawCommand)
    if not isInstalacaoAtiva then
        if args[1] then
            ...
            isInstalacaoAtiva = true
            Citizen.CreateThread(function()
                Citizen.Wait(30000)
                isInstalacaoAtiva = false
            end)
        else
            TriggerEvent("Notify", "vermelho", "Utilize /instalar [xenon, neon ou suspe]", 5000)
        end
    else
        TriggerEvent("Notify", "vermelho", "Aguarde para usar esse comando novamente.", 5000)
    end
end)
        ...
        if ped == GetPedInVehicleSeat(vehicle, -1) then
            ...
            if vehicleSpeed == 0 then
                if vSERVER.checkPermission() then
                    ...
                    TriggerEvent("Notify", "verde", "Você não possui permissão para instalar o módulo de Xenon!", 5000)
                end
            else
                TriggerEvent("Notify", "verde", "O veículo deve estar parado!", 5000)
            end
        else
            TriggerEvent("Notify", "verde", "Você não é o motorista do veículo", 5000)
        end
    else
        TriggerEvent("Notify", "verde", "Você não está em um veículo!", 5000)
    end
        ...
        if ped == GetPedInVehicleSeat(vehicle, -1) then
            ...
            if vehicleSpeed == 0 then
                if vSERVER.checkPermission() then
                    ...
                    TriggerEvent("Notify", "verde", "Você não possui permissão para instalar o módulo de neon!", 5000)
                end
            else
                TriggerEvent("Notify", "verde", "O veículo deve estar parado!", 5000)
            end
...
AddEventHandler('zo_install_suspe_ar', function()
    if vSERVER.checkPermission() then
        local ped = PlayerPedId()
        vehicleInstall = vRP.ClosestVehicle(5)
        if not vSERVER.checkSuspension(vehicleInstall) then
            ...
            if vehicleInstall then
                                                ...
                                                while true do
                                                    ...
                                                    if vehicleInstall == nil or #partsProntas == 4 then
                                                        if vehicleInstall == nil then
                                                            TriggerEvent("Notify", "verde", "Instalação cancelada, você não está próximo do veículo!", 5000)
                                                        end
                                                        return
                                                    end
                                                end
                ...
                TriggerEvent("Notify", "verde","Você não está próximo de um veículo!", 5000)
            end
        else
            TriggerEvent("Notify", "verde","Veículo já possui suspensão a ar", 5000)
        end
    else
        TriggerEvent("Notify", "verde","Você não possui permissão para instalar suspensão a ar!",5000)
    end
end)
