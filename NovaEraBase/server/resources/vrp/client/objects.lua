...
AddEventHandler("objects:Table", function(Table)
    ...
    if ObjectsBlips then
        for Number = 1, 120 do
            ...
            SetBlipAlpha(Blip, ObjectsBlipAlpha)
            SetBlipColour(Blip, ObjectsBlipColour)
        end
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
function targetLabel(x,y,z,Number,Item,Mode,Perm)
	...
	elseif Mode == "Medic" or Mode == "Weapons" or Mode == "Supplies" then
		exports["target"]:AddCircleZone("Objects:"..Number,vec3(x,y,z),2.75,{
			name = "Objects:"..Number,
			heading = 3374176
		},{
			shop = Number,
...
function GetCoordsFromCam(Distance,Coords)
	local PCERJtion = GetGameplayCamRot()
	local Adjusted = vec3((math.pi / 180) * PCERJtion["x"],(math.pi / 180) * PCERJtion["y"],(math.pi / 180) * PCERJtion["z"])
