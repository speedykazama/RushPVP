...
function vRP.ItemAmount(Passport, Item)
    if vRP.Source(Passport) then
        ...
        for _, slotData in pairs(Inventory) do
            if slotData and splitString(slotData.item, "-")[1]  == splitString(Item, "-")[1] then
                totalAmount = totalAmount + slotData.amount
            end
        end
        return totalAmount
    end
...
-- ITEMAMOUNTOFFLINE
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.ItemAmountOffline(Passport, Item)
    local data = vRP.Query("playerdata/GetData", { Passport = Passport, dkey = "Datatable" })
    if data[1] and data[1].dvalue then
        local datatable = json.decode(data[1].dvalue)
        local inventory = datatable.Inventory or {}
        local totalAmount = 0
    ...
    if source and parseInt(Amount) > 0 then
        ...
        if Inventory[Slot] then
            if Inventory[Slot]["item"] == Item then
                ...
                TriggerClientEvent("Notify", source, "vermelho", "O slot já está ocupado por outro item.", 5000)
                return
            end
        else
...
function vRP.MaxItens(Passport, Item, Amount)
    if itemBody(Item) and vRP.Source(Passport) and itemMaxAmount(Item) then
        local currentAmount = vRP.ItemAmount(Passport, Item)
        local maxAmount = itemMaxAmount(Item)

        local UserPremiumPrata = vRP.HasGroup(Passport, "PremiumPrata")
        local UserPremiumOuro = vRP.HasGroup(Passport, "PremiumOuro")
        local UserPremiumPlatina = vRP.HasGroup(Passport, "PremiumPlatina")

        for _, permission in ipairs(PermissionsItensExtrasRestaurante) do
            if vRP.HasService(Passport, permission) and (itemScape(Item) or UserPremiumPrata or UserPremiumOuro or UserPremiumPlatina) then
                maxAmount = maxAmount + ItensExtrasRestaurante
                break
            end
        end

        if UserPremiumPrata then
            maxAmount = maxAmount + ItensExtrasPremiumPrata
        elseif UserPremiumOuro then
            maxAmount = maxAmount + ItensExtrasPremiumOuro
        elseif UserPremiumPlatina then
            maxAmount = maxAmount + ItensExtrasPremiumPlatina
        end

        if currentAmount + parseInt(Amount) > maxAmount then
            return true
        end
    end
    return false
...
-- SAVESERVER2
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("SaveServer2")
AddEventHandler("SaveServer2",function()
	for k,v in pairs(srvData) do
...
-- TAKECHESTREMOVEONLY
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.TakeChestRemoveOnly(Passport, Data, Amount, Slot)
    local source = vRP.Source(Passport)
    local Datatable = vRP.GetSrvData(Data)
    Slot = tostring(Slot)
    Amount = parseInt(Amount)

    if source and Amount > 0 and Datatable[Slot] then
