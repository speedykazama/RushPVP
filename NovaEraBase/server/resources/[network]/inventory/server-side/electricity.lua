...
AddEventHandler("inventory:Electricity", function()
    ...
    if Passport and not Active[Passport] then
        ...
        if os.time() >= Electricity["Blackout"] then
            local Service, Total = vRP.NumPermission(ElectricityPermission)

            if Total >= EelectricityNeed then
                if vRP.ConsultItem(Passport, ElectricityItemRequired, 1) then
                    if vRP.TakeItem(Passport, ElectricityRequired, ElectricityRequiredAmount, true) then
                        if vRP.Memory(source) then
                            ...
                            Electricity["Blackout"] = os.time() + ElectricityCooldownTime
                            ...
                            repeat
                                if os.time() >= parseInt(Active[Passport]) then
                                    ...
                                    vRP.GenerateItem(Passport, ElectricityItem, ElectricityAmount, true)
                                    ...
                                    SetTimeout(10000, function()
                                        ...
                                        TriggerClientEvent("Notify", -1, "azul", "Uma forte explosão em nossa central causou um apagão em toda a cidade, estamos trabalhando para reestabelecer os serviços.", 10000)
                                    end)
                        ...
                        TriggerClientEvent("Notify", source, "amarelo", "Você precisa de <b>3x " .. itemName(ElectricityRequired) .. "</b>.", 5000)
                    end
                else
                    TriggerClientEvent("Notify", source, "amarelo", "Você precisa de <b>1x " .. itemName(ElectricityItemRequired) .. "</b>.", 5000)
                end
            else
