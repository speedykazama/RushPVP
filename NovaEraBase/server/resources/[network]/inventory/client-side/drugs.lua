        ...
        if not IsPedInAnyVehicle(Ped) then
            ...
            if Entitys and not Entity(Entitys)["state"]["Drugs"] then
                ...
                if IsControlJustPressed(1, 38) and GetVehiclePedIsIn(Entitys, true) == 0 and GetGameTimer() >= Timer and vSERVER.CheckDrugs() then
                    ...
                    local transactionTime = 5000

                    while GetGameTimer() - startTime < transactionTime do
                        Wait(1)

                        local Coords = GetEntityCoords(Ped)
                        local EntityCoords = GetEntityCoords(Entitys)

                        DrawText3D(EntityCoords.x, EntityCoords.y, EntityCoords.z + 1.0, randomMessage)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:CLIENT:HALLUCIONEGIC_CHICKEN_WEED
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:CLIENT:HALLUCIONEGIC_PUG_WEED
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- CREATEMPPED
-----------------------------------------------------------------------------------------------------------------------------------------
local function CreateMpPed(ped, custom) end
-----------------------------------------------------------------------------------------------------------------------------------------
-- APPLYBARBERSHOPTOPED
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNetEvent('inventory:client:red_meth', function()
	...
	Wait(2000)
	SetPedShootsAtCoord(pauloPlinioPed, GetEntityCoords(playerPed), true)
	...
	exports["survival"]:Revive(oldHealth)
	SetPedArmour(playerPed, oldArmour)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:CLIENT:HALLUCIONEGIC_LANCAPERFUME
-----------------------------------------------------------------------------------------------------------------------------------------
...
function DrawText3D(x,y,z,Text)
	...
	if onScreen then
		...
		DrawRect(_x,_y + 0.0145,Width,0.03,15,15,15,175)
	end
end