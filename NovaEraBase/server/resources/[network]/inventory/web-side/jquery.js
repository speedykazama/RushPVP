	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				var Blood = $(this).attr("data-Blood");
				var Passport = $(this).attr("data-Passport");
				var Vality = $(this).attr("data-Vality");
				var Premium = $(this).attr("data-Premium");
				var Gemstone = $(this).attr("data-Gemstone");

				contents = `<item>${name} - ${Passport}</item>${description !== "false" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${Name}</r><br>Tipo Sanguíneo: <r>${Blood}</r><br>Diamantes: <r>${Gemstone}</r><br>Validade: <r>${Vality}</r><br>Premium: <r>${Premium}</r></legenda>`;
			}
