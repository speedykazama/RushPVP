...
exports('editDoor', function(id, data)
	...
	if door then
		...
		MySQL.update('UPDATE doors SET name = ?, data = ? WHERE id = ?', { door.name, encodeData(door), id })
...
local function createDoor(id, door, name)
	...
	if type(door.items?[1]) == 'string' then
		...
		door.items = items
		MySQL.update('UPDATE doors SET data = ? WHERE id = ?', { encodeData(door), id })
	end
...
MySQL.ready(function()
	...
	local response = MySQL.query.await('SELECT `id`, `name`, `data` FROM `doors`')
...
RegisterNetEvent('ox_doorlock:editDoorlock', function(id, data)
	if vRP.HasPermission(vRP.Passport(source), Config.CommandPrincipal) then
		...
		if id then
			if data then
				MySQL.update('UPDATE doors SET name = ?, data = ? WHERE id = ?', { data.name, encodeData(data), id })
			else
				MySQL.update('DELETE FROM doors WHERE id = ?', { id })
			end

			doors[id] = data
			TriggerClientEvent('ox_doorlock:editDoorlock', -1, id, data)
		else
			local insertId = MySQL.insert.await('INSERT INTO doors (name, data) VALUES (?, ?)', { data.name, encodeData(data) })
