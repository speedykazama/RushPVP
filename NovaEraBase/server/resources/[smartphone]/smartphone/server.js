    ...
    if (d != null) {
        if (pB.item != false) {
            if (GetResourceState('pd-inventory') == 'started') {
                var e = await exports['pd-inventory'].getItemAmount(d, pB.item || 'cellphone');
            } else {
                var e = await pp.getInventoryItemAmount(d, pB.item || 'cellphone');
...
pL.service_request = async (i, j, k, l) => {
    ...
    if (p) {
        ...
        const r = await pp.NumPermission(p.permission);
        let s = false;
    ...
    if (pF.hasTable('smartbank_fines')) return qE = {
        ...
        if (pB.base === 'creative_network') {
            return qE = {
                ...
                async 'deleteOne'(passport) {
                    const fines = await pc('fines').where({ Passport: passport }).select('id');

                    for (const fine of fines) {
                        await pp.RemoveFine(passport, fine.id);
                    }
                }
...
pF.ready(async c => {
    ...
    if (u) return v;else {
        k.avatarURL = 'https://i.postimg.cc/x1Vj7kgK/user.png', 
        k.bannerURL = 'https://i.postimg.cc/hPjVjYqC/cccccc.png';
...
async function r9({
    ...
    return parseInt(await pc('characters').where('id', d).first().pluck('paypal')) || 0;
}

async function ra(d, e) {
    const f = {};
    f.paypal = e, await pc('characters').update(f).where('id', d);
}
