...
-- PREPARE
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.Prepare('ifood/create', [[CREATE TABLE IF NOT EXISTS smartphone_ifood_orders(id VARCHAR(10) PRIMARY KEY,Passport INT,worker_id INT,store_id INT,total INT,fee INT,rate TINYINT DEFAULT 0,created_at INT)]])
vRP.Prepare('ifood/insert', [[INSERT INTO smartphone_ifood_orders(id, Passport, worker_id, store_id, total, fee, created_at)VALUES (@id, @Passport, @worker_id, @store_id, @total, @fee, UNIX_TIMESTAMP())]])
...
function cancelOrder(order, refund)
  orders[order.id] = nil
  if refund then
    vRP.GiveBank(order.customer.id, order.total)
    workers[order.worker] = nil
  else
    vRP.GiveBank(vRP.Passport(order.worker), order.fee)
...
expose('ifood_set_rate', function(source, id, rate)
  ...
  vRP.Query('ifood/set_rate', { id = id, rate = rate })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- GETPRICE
-----------------------------------------------------------------------------------------------------------------------------------------
...
expose('ifood_create_order', function(source, id, cart, distance)
  ...
  CreateThread(function()
    ...
    if not order.worker then
      orders[order.id] = nil
      vRP.GiveBank(Passport, order.total)
    end
  end)
...
expose('ifood_finish_route', function(source, code)
  ...
  vRP.GiveBank(Passport, order.fee)
...
AddEventHandler('smartphone:isReady', function()
  exports.smartphone:createApp(
    'ifood', 
    'iFood', 
    'nui://smartphone-jobs/web-side/static/media/ifood.webp',
    'nui://smartphone-jobs/web-side/index.html#/ifood'
  )
end)